...
public class LoginServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		//get request body -- request.getInputStream() and user ObjectMapper to read value
		//consult service layer (Call methods from service class) - user username/password valid?
		//if so, return JSON of user, otherwise null JSON String
		
		ObjectMapper mapper = new ObjectMapper();
		User inputUser = mapper.readValue(req.getInputStream(), User.class);
		log.info("ADDING USER: " + inputUser.toString());
		
		User loggedUser = service.getByUsername(inputUser.getUsername()); //method from user class
		String out = "";
